/**
 * Seed demo account using the Firebase client SDK (no service account needed).
 *
 * Usage:
 *   node scripts/seedDemoAccountClient.mjs
 *
 * Env overrides: DEMO_EMAIL, DEMO_PASSWORD, DEMO_FIRST_NAME, DEMO_LAST_NAME
 */
...
function synthesizeTextAnswer() {
  return "Early-stage business with a lean team and improving weekly operating rhythm.";
}

function buildAnswersForSection(section, biasOverride) {
  const answers = {};
  let sectionScore = 0;
  const order = typeof section.order === "number" ? section.order : 0;
  const bias = biasOverride || categoryBiasForSectionOrder(order);
  ...
  (section.questions || []).forEach((q) => {
    if (q.type === "multipleChoice") {
      ...
      answers[q.id] = { answer: synthesizeTextAnswer(), weight: 0 };
    }
  });
...
async function ensureDemoAuthUser() {
  try {
    ...
    console.log(`Signed in existing demo user: ${cred.user.uid}`);
    return cred.user;
  } catch (error) {
  ...
  await updateProfile(cred.user, { displayName: `${DEMO_FIRST} ${DEMO_LAST}` });
  console.log(`Created demo auth user: ${cred.user.uid}`);
  return cred.user;
}

async function upsertDemoUserDoc(uid) {
  ...
  await setDoc(doc(db, "usernames", username.toLowerCase()), { uid, username }, { merge: true });
}

async function seedUserAssessment(uid, email, biasOverride) {
  ...
  const existingSnap = await getDocs(query(collection(db, "sectionResults"), where("userId", "==", uid)));
  ...
  for (const section of sections) {
    const { answers, sectionScore } = buildAnswersForSection(section, biasOverride);
    ...
    const existing = existingBySectionName.get(section.title);
    if (existing) {
  ...
  console.log(`Seeded assessment for ${email} (${sections.length} sections)`);
}

async function ensureMockClient({ firstName, lastName, email, password, bias }) {
  let user;
  try {
  ...
  await seedUserAssessment(user.uid, email, bias);
}

async function main() {
  console.log("Seeding demo account via client SDK...");
  const demoUser = await ensureDemoAuthUser();
  await upsertDemoUserDoc(demoUser.uid);
  await seedUserAssessment(demoUser.uid, DEMO_EMAIL);
  ...
  for (const client of mockClients) {
    await ensureMockClient({ ...client, password: "TestUser2026!" });
  }
